...
if ($content -match '<artifactId>remoteagent</artifactId>\s*<version>(\d+)\.(\d+)\.(\d+)</version>') {
    $major = $matches[1]
    $minor = $matches[2]
    ...
    $content = $content -replace '(<artifactId>remoteagent</artifactId>\s*<version>)\d+\.\d+\.\d+(</version>)', "`${1}$newVersion`${2}"
